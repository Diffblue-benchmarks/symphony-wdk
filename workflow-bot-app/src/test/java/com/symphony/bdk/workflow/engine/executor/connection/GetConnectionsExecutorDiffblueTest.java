...
class GetConnectionsExecutorDiffblueTest {
   ...
   *
   * <ul>
   *   <li>Then calls {@link ConnectionService#listConnections(ConnectionStatus, List)}.
   * </ul>
   *
   * <p>Method under test: {@link GetConnectionsExecutor#execute(ActivityExecutorContext)}
   */
  @Test
  @DisplayName(
      "Test execute(ActivityExecutorContext); then calls listConnections(ConnectionStatus, List)")
  ...
  void testExecute_thenCallsListConnections() {
    ...
    when(connectionService.listConnections(
            Mockito.<ConnectionStatus>any(), Mockito.<List<Long>>any()))
