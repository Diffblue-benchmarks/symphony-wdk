...
class GetStreamMembersExecutorDiffblueTest {
  ...
  @Test
  @DisplayName(
      "Test execute(ActivityExecutorContext); given GetStreamMembers getLimit() return one; then calls bdk()")
  @Tag("ContributionFromDiffblue")
  @ManagedByDiffblue
  @MethodsUnderTest({"void GetStreamMembersExecutor.execute(ActivityExecutorContext)"})
  void testExecute_givenGetStreamMembersGetLimitReturnOne_thenCallsBdk() {
    // Arrange
   ...
   *
   * <ul>
   *   <li>Then calls {@link GetStreamMembers#getId()}.
   * </ul>
   *
   * <p>Method under test: {@link GetStreamMembersExecutor#execute(ActivityExecutorContext)}
   */
  @Test
  @DisplayName("Test execute(ActivityExecutorContext); then calls getId()")
  @Tag("ContributionFromDiffblue")
  @ManagedByDiffblue
  @MethodsUnderTest({"void GetStreamMembersExecutor.execute(ActivityExecutorContext)"})
  void testExecute_thenCallsGetId() {
    // Arrange
    GetStreamMembers getStreamMembers = mock(GetStreamMembers.class);
    when(getStreamMembers.getId()).thenThrow(new IllegalArgumentException());
    when(getStreamMembers.getLimit()).thenReturn(null);
    when(getStreamMembers.getSkip()).thenReturn(1);
    when(getStreamMembers.getStreamId()).thenReturn("42");
    ...
    assertThrows(IllegalArgumentException.class, () -> getStreamMembersExecutor.execute(execution));
    verify(execution).getActivity();
    verify(getStreamMembers).getId();
    verify(getStreamMembers, atLeast(1)).getLimit();
    verify(getStreamMembers).getSkip();
    verify(getStreamMembers).getStreamId();
  }
