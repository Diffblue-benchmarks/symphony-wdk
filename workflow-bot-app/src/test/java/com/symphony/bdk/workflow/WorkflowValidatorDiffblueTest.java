...
class WorkflowValidatorDiffblueTest {
   ...
   *
   * <ul>
   *   <li>Then calls {@link EventWithTimeout#getActivityCompleted()}.
   * </ul>
   *
   * <p>Method under test: {@link WorkflowValidator#validateFirstActivity(BaseActivity, Event,
   * String)}
  ...
  @Test
  @DisplayName(
      "Test validateFirstActivity(BaseActivity, Event, String); then calls getActivityCompleted()")
  @Tag("ContributionFromDiffblue")
  @ManagedByDiffblue
  @MethodsUnderTest({"void WorkflowValidator.validateFirstActivity(BaseActivity, Event, String)"})
  void testValidateFirstActivity_thenCallsGetActivityCompleted() {
    // Arrange
    EventWithTimeout on = mock(EventWithTimeout.class);
    when(on.getActivityCompleted())
        .thenThrow(new InvalidActivityException("42", "An error occurred"));
    ...
    assertThrows(
        InvalidActivityException.class,
        () -> WorkflowValidator.validateFirstActivity(activity, event, "42"));
    verify(on).getActivityCompleted();
  }
