...
class JoinActivityNodeBuilderClaude_typeTest {

  // Tests for type() method

  @Test
  void type_shouldReturnJoinActivity() {
    // Given: An instance of JoinActivityNodeBuilder
    JoinActivityNodeBuilder builder = new JoinActivityNodeBuilder();

    // When: Calling type()
    WorkflowNodeType result = builder.type();

    // Then: Should return JOIN_ACTIVITY
  ...
  void type_shouldReturnSpecificEnumValue() {
    ...
    // Then: Should specifically be JOIN_ACTIVITY and not any other enum value
  ...
  void type_multipleInstances_shouldReturnSameEnumConstant() {
    // Given: Multiple instances of JoinActivityNodeBuilder
  ...
  void type_shouldHaveCorrectOrdinal() {
    ...
    // Then: Should have the correct ordinal (6th position in the enum)
    assertThat(result.ordinal()).isEqualTo(6);
  }
  ...
  void type_shouldHaveCorrectName() {
    ...
    // Then: The enum name should be "JOIN_ACTIVITY"
    assertThat(result.name()).isEqualTo("JOIN_ACTIVITY");
  }
  ...
  void type_shouldBeCompatibleWithEnumValueOf() {
    ...
    WorkflowNodeType result = builder.type();
    WorkflowNodeType fromValueOf = WorkflowNodeType.valueOf("JOIN_ACTIVITY");
